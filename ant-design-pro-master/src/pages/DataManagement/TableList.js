...
/*******加载本目录下的models目录下的DMPro******/
@connect(({DMPro, loading}) => ({
  DMPro,
  loading: loading.models.DMPro,
}))
@Form.create()
class TableList extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'DMPro/fetchBasic',//调用proflie下的fetchBasic函数
      payload: 1
    });
  }
...
//将UpdateForm隐藏
  ...
  getInputValue = () => {
    ...
    const {dispatch} = this.props;
    dispatch({
      type: 'DMPro/SearchByid',//fetchBasic',//调用proflie下的fetchBasic函数
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{md: 8, lg: 24, xl: 48}}>
          <Col md={8} sm={24}>
            <FormItem label="查询">
              {getFieldDecorator('name')(<Input placeholder="请输入用户ID" onChange={this.bindInput.bind(this)}></Input>)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
